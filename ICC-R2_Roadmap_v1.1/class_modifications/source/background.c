...
int background_functions(
  ...
  /* === ICC-R2 COMPONENT (START) === */
  if (pba->has_icc == _TRUE_) {
    /* a is already a function argument, do NOT redeclare */
    double w_icc = -0.95 + pba->mu_icc * a;
    double w_eff = -0.95; /* stable scaling for rho */
    
    /* CRITICAL FIX: CLASS uses units [Mpc^-2]. Critical density today = H0^2.
       mu_icc is Omega_icc, so rho = Omega * H0^2 * a^(-3(1+w)) */
    double rho_icc = pba->mu_icc * pba->H0 * pba->H0 * pow(a, -3.0*(1.0+w_eff));
    double p_icc   = w_icc * rho_icc;

    /* Add to local totals (CLASS recalculates H automatically) */
    rho_tot += rho_icc;
    p_tot   += p_icc;

    /* DEBUG: print once at late times to verify scaling */
    static int icc_dbg = 0;
    if (!icc_dbg && a > 0.99) {
      printf("[ICC-VERIFY] a=%.3f | mu=%.4f | H0=%.6e | rho_icc=%.6e | rho_tot=%.6e\n",
             a, pba->mu_icc, pba->H0, rho_icc, rho_tot);
      fflush(stdout);
      icc_dbg = 1;
    }
  }
  /* === ICC-R2 COMPONENT (END) === */
...
int background_init(
  ...
  /* ICC-R2: precision -> background */
  pba->mu_icc = ppr->mu_icc;
  pba->has_icc = ppr->has_icc;  
...
int background_indices(
  ...
  /* === ICC-R2 indices === */
  class_define_index(pba->index_bg_rho_icc, pba->has_icc, index_bg, 1);
  class_define_index(pba->index_bg_p_icc, pba->has_icc, index_bg, 1);
  /* === END ICC-R2 === */
  /* - index for Lambda */
